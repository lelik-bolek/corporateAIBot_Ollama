...
def main():
    ...
    test_queries = [
    # 1. Семантический запрос (проверка понимания назначения и концепции)
    "Для каких устройств и задач разработан легковесный агент MiNiFi?",
    
    # 2. Фактологический запрос с сопоставлением (проверка поиска по табличным характеристикам)
    "Какие минимальные системные требования по CPU и RAM нужны для Apache MiNiFi?",
    
    # 3. Сравнительный семантический запрос (различия реализаций)
    "В чем различия между агентом MiNiFi на Java и агентом на C++ по потреблению памяти?",
    
    # 4. Поиск точных параметров конфигурации и путей (проверка работы с кодовыми сущностями)
    "Какой порт входного сокета Site to Site настраивается в nifi.properties?",
    
    # 5. Процедурный запрос (шаги настройки интеграции)
    "Как настроить группу удаленных процессов RPG для подключения к входному порту NiFi?",
    
    # 6. Контроль отсечения оффтопа (скор должен быть ниже порога SIMILARITY_THRESHOLD)
